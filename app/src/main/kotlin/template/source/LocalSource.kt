...
import template.utils.RarContentProvider
import template.utils.ZipContentProvider
        ...
        val mangas = mangaDirs.map { mangaDir ->
            SManga.create().apply {
                ...
                if (thumbnail_url == null) {
                    val chapters = fetchChapterList(this).blockingFirst()
                    if (chapters.isNotEmpty()) {
                        val uri = fetchPageList(chapters.last()).blockingFirst().firstOrNull()?.uri
                        if (uri != null) {
