...
import template.extensions.asObservable
import template.extensions.asObservableSuccess
import template.extensions.selectInt
import template.extensions.selectText
import template.network.GET
...
class MyanimelistApi(private val client: OkHttpClient, username: String, password: String) {
    ...
    fun search(query: String, username: String): Single<List<TrackSearch>> {
        return if (query.startsWith(PREFIX_MY)) {
            ...
            client.newCall(GET(getSearchUrl(query), headers))
                    .asObservable()
                    .map {
                        Jsoup.parse(Parser.unescapeEntities(it.body()!!.string(), false), "", Parser.xmlParser())
                    }
                    .flatMap {
                        Observable.fromIterable(it.select("entry"))
                    }
                    .filter {
                        it.select("type").text() != "Novel"
                    }
                    .map {
    ...
    fun getList(username: String): Single<List<TrackSearch>> {
        return client
                ...
                .flatMap {
                    Observable.fromIterable(it.select("manga"))
                }
                .map {
                    TrackSearch.create(TrackManager.MYANIMELIST).apply {
    ...
    fun getAddUrl(track: Track) = Uri.parse(baseUrl).buildUpon()
            .appendEncodedPath("api/mangalist/add")     // That's how Uri.Builder works. It encodes non-safe URL characters with special meaning to their %xx hex values
            .appendPath("${track.media_id}.xml")
            .toString()
